...
### sourceが空 or method: manual の場合（3段フォールバック）

**いきなりローカル配置を案内してはいけない。** まずユーザーに収集元を質問する:

```
「{clientName} の請求書はどこから収集しますか？
 1. Google Drive（フォルダを指定）
 2. Gmail（メール添付）
 3. バクラク（API取得）
 4. 手元にPDFがある（ローカル配置）
 5. その他（具体的に教えてください）」
...
**回答に基づく処理:**
- 1〜3 → 該当する `--source` オプション付きで collect.ts を実行
- 4 → ローカル配置を案内（下記「手動配置」参照）
- 5 → 回答内容をもとに最適な方法を判断

**リファレンス更新の提案:**
ユーザーの回答で新しい情報が得られた場合、収集完了後に以下を提案する:
```
「この収集元情報をリファレンスに保存しますか？
 次回から自動で同じ方法で収集されるようになります。」
```
→ 承認されたら REF_FILE の該当フィールドを Edit で更新し、リファレンスリポへのコミット・プッシュも提案する

### 手動配置（ユーザーが「手元にPDFがある」と回答した場合）
